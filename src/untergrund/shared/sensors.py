    ...
    def build_wrappers(core: Callable[..., T]) -> "SensorStep[T]":
        ...
        def select(
            *,
            ...
            def with_kwargs_select(**kw: Any) -> "SensorStep[T]":
                # 1) Core parametrisieren (ohne Selektion)
                parametrized = with_kwargs(**kw)  # ergibt einen neuen *unselektierten* Step
                # 2) identische Selektion erneut anwenden
                return parametrized.select(
                    include=include, exclude=exclude, regex=regex, predicate=predicate
                    )
            ...
            selective_wrapper.with_kwargs = with_kwargs_select  # type: ignore[attr-defined]
    ...
    def build_wrappers(core: Callable[..., None]) -> "SensorInspector[object]":
        ...
        def select(
            *,
            ...
            def with_kwargs_select(**kw: Any) -> "SensorInspector[object]":
                parametrized = with_kwargs(**kw)
                return parametrized.select(
                    include=include, exclude=exclude, regex=regex, predicate=predicate
                )
